...
from model.models.core import normalize, embedding
from model.models.core.fish import \
    multi_head_attention_blocks as admix_multi_head_attention_blocks
from model.models.abs.sas import AbsSasRec

from model.logging import get_logger
class model(AbsSasRec):
    # """
    ...
    def __init__(self, sess, params, n_users, n_items):
        super(model, self).__init__(sess, params,
                                     n_users, n_items)
    ...
    def build_feedict(self, batch, is_training=True):
        ...
        if is_training is True:
            ...
            feed_dict[self.next_item] = batch[2][-1].reshape(-1,1)
            self.update_seq_ids = batch[1]
        else:
            ...
            ##feed_dict[self.next_item] = batch[2][-1].reshape(-1,1)
        return feed_dict
    # 计算序列中每个 item 和 next_item 的余弦相似度
    def cosine_similarity(self, vec1, vec2):
        # 在第二维度扩展 vec1，使其形状与 vec2 可广播
        vec1 = tf.expand_dims(vec1, axis=1)  # [batch_size, 1, embedding_dim]

        # 点积
        dot_product = tf.reduce_sum(vec1 * vec2, axis=-1)  # [batch_size, sequence_length]

        # 模长
        norm1 = tf.norm(vec1, axis=-1)  # [batch_size, 1]
        norm2 = tf.norm(vec2, axis=-1)  # [batch_size, sequence_length]
    ...
    def replace_some_item(self):
        # 转换为 NumPy 数组
        vec1 = self.nonscale_input_seq
        vec2 = self.next_item_embeding
        vec2 = tf.squeeze(vec2, axis=1)
        # self.logger.info("str1 " + str(vec1.shape))
        # self.logger.info("str2 " + str(vec2.shape))
        # 相似度计算
        similarities = self.cosine_similarity(vec2, vec1)

        #similarities = tf.squeeze(similarities,axis=1)
        #similarities = tf.squeeze(similarities, axis=0)
        #self.logger.info("str2 " + str(similarities.shape))
        # 获取前 n 个高相似度的索引和分数
        n = 15

        high_similarity_indices = tf.argsort(-similarities, axis=1)[::,0:n]  # 高相似度，取前 n 大
        high_similarity_values = tf.gather(tf.reshape(similarities, [-1]), high_similarity_indices)
        #self.logger.info("sim " + str(tf.argsort(similarities, axis=1).shape))

        # 获取后 n 个低相似度的索引和分数
        low_similarity_indices = tf.argsort(similarities, axis=1)[::,0:n] # 低相似度，取前 n 小
        low_similarity_values = tf.gather(tf.reshape(similarities, [-1]), low_similarity_indices)

        # 提取高相似度的 items
        high_similarity_items = tf.gather(self.seq_ids, high_similarity_indices,axis=1)


        #high_similarity_items = tf.squeeze(high_similarity_items)
        # 替换低相似度的 items
        updated_watch_movie = np.array(self.update_seq_ids)  # 转为张量

        ## 替换部分没有写好
        # self.logger.info("batch.size" + str(self.next_item.shape))
        # self.logger.info("updated_watch_movie.shape" + str(updated_watch_movie.shape))
        # self.logger.info("low_similarity_indices" + str(low_similarity_values.shape))
        # self.logger.info("high_similarity_items" + str(high_similarity_items.shape))

        # batch_size = tf.shape(updated_watch_movie)[0].value
        # for i in range(batch_size):
        #     indices = low_similarity_indices[i]
        #     updates = high_similarity_items[i]
        #     updated_watch_movie = tf.tensor_scatter_nd_update(
        #         updated_watch_movie,
        #         tf.expand_dims(indices, axis=-1),
        #         updates
        #     )
        #self.logger.info(updated_watch_movie[0])
        high_similarity_items = tf.reshape(high_similarity_items,shape=[-1,-1,self.seqlen])
        updated_watch_movie = tf.tensor_scatter_nd_update(
            updated_watch_movie,
            tf.expand_dims(low_similarity_indices, axis=-1),  # 索引需要为二维
            high_similarity_items
        )
        #self.logger.info("low_similarity_indices",type(low_similarity_values))
        #self.logger.info("high_similarity_items", type(high_similarity_items))
        #updated_watch_movie[low_similarity_indices] = high_similarity_items
        # 更新历史嵌入
        self.nonscale_input_seq = tf.nn.embedding_lookup(self.item_embedding_table, updated_watch_movie)


        return updated_watch_movie
    ...
    def _create_placeholders(self):
        super(model, self)._create_placeholders()
    ...
    def _create_variables(self, reuse=None):
        super(model, self)._create_variables(reuse=reuse)
    ...
    def _create_net_inference(self, name, reuse=None):
        ...
        super(model, self)._create_net_inference(name, reuse=reuse)
        with tf.compat.v1.variable_scope('shared_input_comp',
            ...
            self.next_item_embeding = tf.nn.embedding_lookup(
                self.item_embedding_table, self.next_item)
